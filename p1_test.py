...
class TestChatWithBot(unittest.TestCase):
    ...
    def test_chat_with_bot_exit_saves_history(self, mock_save, mock_input):
        ...
        with patch.object(type(llm), 'invoke', return_value=MagicMock(content='Yes! Which outlet are you referring to?')):
    ...
    def test_chat_with_bot_handles_empty_input(self, mock_save, mock_input):
        ...
        with patch.object(type(llm), 'invoke', return_value=MagicMock(content='Ah yes, the SS 2 outlet opens at 9.00AM.')):
            session_id = 'test-session-id'
